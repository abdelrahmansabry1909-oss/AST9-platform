...
- **Verification plan** — exactly how §3 will be satisfied for this phase.
...
## 3. Required Verification After Every Phase
...
## 4. Mandatory Guard-Skills Review
...
- **When relevant:**
  ...
  - `test-guard` — **only if real in-repo tests exist** (none today; skip until they do).
  - `wp-guard` / `woo-guard` — **not applicable** unless WordPress/WooCommerce is introduced.

**Order of application (strict):**
1. **Mechanical verification first** (§3 — `node --check`, build, smoke).
...
## 5. Required Guard Prompt After Every Phase
...
- **files changed**,
- **verification results** (§3 outcomes),
