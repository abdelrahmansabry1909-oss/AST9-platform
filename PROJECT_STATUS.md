...
**Live:** `origin/main` @ `73be2be` · GitHub Pages <https://abdelrahmansabry1909-oss.github.io/AST9_HUB/> (auto-deployed by `.github/workflows/deploy.yml`).
**Supabase:** `byquokhcbagofshsclfy` (eu-central-1, Postgres 17) · **73 migrations** · **11 edge functions** · **390 unit tests** · **owner-only admin verified live (`admin_count = 1`)**.
...
| MA | Full-body movement analysis — levelled gait engines, cross-region integration engine, folding result panels, scapular activation chart. Frontend only; no DB/edge/auth change. **Implemented by Claude at the owner's direct instruction**, which is a deliberate exception to the agent split above. | PR #207 (`b07a8b4`), PR #208 (`cb6a553`), PR #209 (`b98bf9a`); `js/integrationEngine.js`, `js/panelFold.js`, `src/neucore/simulation/ShoulderActivation*.js`; DEV_LOG *NeuCore movement scoring*; ISSUE_LOG #26–#28 |
| MB | The assessment survives being saved — 21 columns added to `rehab_objective_assessments` **and applied to production**, the write path raised from 45 to 70 columns (4 of which had columns since the table was created and were silently discarded), the upper body wired to the 3D body map on Neumann lumbar norms, and a failed save no longer reported as success. | PR #210 (`fe86f45`), PR #211 (`7d90139`), PR #212 (`73be2be`); migration `20260809000000`; KNOWN_LIMITATIONS L21 (closed) / L22 (resolved); ISSUE_LOG #29; DECISIONS D18 |
