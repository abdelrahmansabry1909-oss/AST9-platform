...
The repository contains **69** forward migrations. By the
`<version>_<name>.sql` / `<version>_<name>_down.sql` naming convention,
**34 are paired** and **35 lack a paired down-file**.

Rollback files are split across two directories:
`supabase/rollbacks/` contains **19** files, and
`supabase/migrations/rollbacks/` contains **15** files. There are **0 orphan**
