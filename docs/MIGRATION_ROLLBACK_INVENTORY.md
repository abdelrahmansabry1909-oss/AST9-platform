...
The repository contains **73** forward migrations. By the
`<version>_<name>.sql` / `<version>_<name>_down.sql` naming convention,
**38 are paired** and **35 lack a paired down-file**.

Rollback files are split across two directories:
`supabase/rollbacks/` contains **19** files, and
`supabase/migrations/rollbacks/` contains **19** files. There are **0 orphan**
...
| `20260804000000_server_side_package_prices.sql` | paired | `supabase/rollbacks/` | Paired down migration is present. |
...
| `20260809000000_upper_body_rom_columns.sql` | paired | `supabase/migrations/rollbacks/` | **APPLIED TO PRODUCTION 2026-08-27** (owner-approved; `execute_sql`, version pinned by hand). 72 → 93 columns, 17 rows hash-verified unchanged, policies/indexes/constraints unchanged, no new advisor finding. Paired down migration is present. Adds **21** nullable columns to `rehab_objective_assessments` for the shoulder-abduction, thoracic, cervical, elbow, wrist and lumbar measurements the form has always collected and never stored (KNOWN_LIMITATIONS L21). Additive only — no existing column, constraint, policy, function or row is modified, and both row-scoped policies plus the table-level grants cover the new columns automatically. **Apply before the frontend write path ships:** the insert in `js/dashboard.js` is all-or-nothing and its failure is swallowed by a `console.warn`, so the reverse order would silently lose every objective assessment. The rollback drops the 21 columns and is **destructive once coaches have saved into them** — usable only while they are still empty. |
