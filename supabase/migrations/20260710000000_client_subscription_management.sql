...
DECLARE
  ...
  v_end    date := COALESCE(p_end, (v_start + make_interval(months => p_months))::date);
  v_id     uuid;
BEGIN
  ...
  -- validation (honest errors, never silent success)
...
BEGIN
  ...
  IF p_months IS NULL OR p_months < 1 OR p_months > 60 THEN
    RAISE EXCEPTION 'months must be between 1 and 60';
  END IF;
  IF p_status IS NULL OR p_status NOT IN ('active', 'pending', 'expired', 'cancelled') THEN
