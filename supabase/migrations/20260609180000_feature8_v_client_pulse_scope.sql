-- ═══════════════════════════════════════════════════════════════
-- NeuCore — Feature 8 · Coach-visibility containment (Option B)
--
-- Scopes the read-only v_client_pulse view so each caller only sees
-- the clients they are entitled to:
--   • admin   → all clients            (public.is_admin())
--   • client  → self only              (client_id = auth.uid())
--   • coach   → ASSIGNED clients only  (profiles.assigned_coach = auth.uid())
--
-- WHY: every per-client source table (workout_sessions, daily_routine_logs,
-- progress_snapshots, subscriptions, client_programs) is already RLS-scoped
-- to "is_admin() OR assigned_coach = auth.uid()", but `profiles` SELECT is
-- global for staff ("Coaches and admins read all profiles"). Because the
-- view's client set derives from profiles (via v_client_progression), a coach
-- otherwise gets a row for EVERY client (id enumeration). This adds the same
-- assigned-coach predicate the sibling tables already use, so the view's
-- "coach = assigned" contract is actually enforced.
--
-- SCOPE: this is the Feature 8 containment fix ONLY. It does NOT change the
-- global `profiles` policy; the broader coach-PII exposure in clients.js /
-- dashboard / pickers remains a separate, tracked hardening phase
-- (see PROJECT_STATUS.md → "Deferred: global coach profile visibility").
--
-- Additive / idempotent (CREATE OR REPLACE; identical columns, grants,
-- security_invoker). Rollback: supabase/rollbacks/20260609180000_*_down.sql
-- ═══════════════════════════════════════════════════════════════
